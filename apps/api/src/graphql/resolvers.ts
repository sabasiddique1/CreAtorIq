...
export const resolvers = {
  Query: {
    ...
    recentlyViewedContent: async (_: any, { limit = 10 }: { limit?: number }, context: GraphQLContext) => {
      if (!context.user) {
        throw new Error("Unauthorized")
      }

      // Get recently viewed content items for this user, sorted by most recent view
      const views = await ContentViewModel.find({ userId: context.user.userId })
        .sort({ viewedAt: -1 })
        .limit(limit)

      const contentIds = views.map((view) => view.contentItemId)
      ...
      const contentMap = new Map(
        contentItems.map((item: any) => [item._id.toString(), item.toObject()])
      )

      // Create a map of view timestamps
      const viewMap = new Map(
        views.map((view: any) => [view.contentItemId.toString(), view.viewedAt])
      )

      // Filter and sort by view time
      const accessibleContent = []
      for (const view of views) {
        const contentId = view.contentItemId.toString()
        ...
        if (item.isPremium) {
          ...
          if (item.requiredTier) {
            ...
            if (!accessibleTiers.includes(item.requiredTier)) {
              continue
            }
          }
        }
        ...
        accessibleContent.push({
          ...
          isViewed: true,
          isNew: false,
          viewedAt: viewMap.get(contentId),
        })
      }
